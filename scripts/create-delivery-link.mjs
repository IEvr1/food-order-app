...
if (!secret) {
  console.error("Set DASHBOARD_LINK_SECRET before creating a production delivery link.");
  process.exitCode = 1;
} else {
  ...
  const link = `${baseUrl}/dashboard/delivery?code=${encodeURIComponent(code)}`;

  if (permanent) {
    console.log("Delivery link (no expiry):");
  } else {
    console.log(`Delivery link (${Math.round(ttlSeconds / 86400)} days):`);
  }
  console.log(link);
  if (expiresAt) {
